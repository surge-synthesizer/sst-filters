...
{
    f->C[0] = A(f->C[0], f->dC[0]); // F1
    f->C[1] = A(f->C[1], f->dC[1]); // Q1

    __m128 L = A(f->R[1], M(f->C[0], f->R[0]));
    __m128 H = S(S(in, L), M(f->C[1], f->R[0]));
    __m128 B = A(f->R[0], M(f->C[0], H));

    __m128 L2 = A(L, M(f->C[0], B));
    __m128 H2 = S(S(in, L2), M(f->C[1], B));
    __m128 B2 = A(B, M(f->C[0], H2));

    f->R[0] = M(B2, f->R[2]);
    f->R[1] = M(L2, f->R[2]);

    f->C[2] = A(f->C[2], f->dC[2]);
    const __m128 m01 = F(0.1f);
    f->R[2] = MAX(m01, S(m128_one, M(f->C[2], M(B, B))));

    f->C[3] = A(f->C[3], f->dC[3]); // Gain
    return M(L2, f->C[3]);
}
...
inline __m128 SVFLP24Aquad(QuadFilterUnitState *__restrict f, __m128 in)
{
    ...
    L = A(f->R[4], M(f->C[0], f->R[3]));
    H = S(S(in, L), M(f->C[1], f->R[3]));
    B = A(f->R[3], M(f->C[0], H));
...
{
    f->C[1] = A(f->C[1], f->dC[1]);                     // K2
    f->C[3] = A(f->C[3], f->dC[3]);                     // Q2
    __m128 f2 = S(M(f->C[3], in), M(f->C[1], f->R[1])); // Q2*in - K2*R1
    __m128 g2 = A(M(f->C[1], in), M(f->C[3], f->R[1])); // K2*in + Q2*R1

    f->C[0] = A(f->C[0], f->dC[0]);                     // K1
    f->C[2] = A(f->C[2], f->dC[2]);                     // Q1
    __m128 f1 = S(M(f->C[2], f2), M(f->C[0], f->R[0])); // Q1*f2 - K1*R0
    __m128 g1 = A(M(f->C[0], f2), M(f->C[2], f->R[0])); // K1*f2 + Q1*R0

    f->C[4] = A(f->C[4], f->dC[4]); // V1
    f->C[5] = A(f->C[5], f->dC[5]); // V2
    f->C[6] = A(f->C[6], f->dC[6]); // V3
    __m128 y = A(A(M(f->C[6], g2), M(f->C[5], g1)), M(f->C[4], f1));
...
inline __m128 IIR12Bquad(QuadFilterUnitState *__restrict f, __m128 in)
{
    ...
    f->C[7] = A(f->C[7], f->dC[7]); // Clipgain
    const __m128 m01 = F(0.1f);

    f->R[2] = MAX(m01, S(m128_one, M(f->C[7], M(y, y))));
...
{
    f->C[0] = A(f->C[0], f->dC[0]); // E1 * sc
    f->C[1] = A(f->C[1], f->dC[1]); // E2 * sc
    f->C[2] = A(f->C[2], f->dC[2]); // -E1 / sc
    f->C[3] = A(f->C[3], f->dC[3]); // -E2 / sc
    f->C[4] = A(f->C[4], f->dC[4]); // C1
    f->C[5] = A(f->C[5], f->dC[5]); // C2
    f->C[6] = A(f->C[6], f->dC[6]); // D

    __m128 y = A(A(M(f->C[4], f->R[0]), M(f->C[6], in)), M(f->C[5], f->R[1]));
    __m128 t = A(in, A(M(f->C[2], f->R[0]), M(f->C[3], f->R[1])));

    __m128 s1 = A(M(t, f->C[0]), f->R[0]);
    __m128 s2 = S(_mm_setzero_ps(), A(M(t, f->C[1]), f->R[1]));
...
inline __m128 IIR12CFCquad(QuadFilterUnitState *__restrict f, __m128 in)
{
    ...
    f->C[0] = A(f->C[0], f->dC[0]); // ar
    f->C[1] = A(f->C[1], f->dC[1]); // ai
    f->C[2] = A(f->C[2], f->dC[2]); // b1
    f->C[4] = A(f->C[4], f->dC[4]); // c1
    f->C[5] = A(f->C[5], f->dC[5]); // c2
    f->C[6] = A(f->C[6], f->dC[6]); // d
    ...
    __m128 y = A(A(M(f->C[4], f->R[0]), M(f->C[6], in)), M(f->C[5], f->R[1]));
    __m128 s1 = A(M(in, f->C[2]), S(M(f->C[0], f->R[0]), M(f->C[1], f->R[1])));
    __m128 s2 = A(M(f->C[1], f->R[0]), M(f->C[0], f->R[1]));
...
inline __m128 IIR12CFLquad(QuadFilterUnitState *__restrict f, __m128 in)
{
    ...
    __m128 y = A(A(M(f->C[4], f->R[0]), M(f->C[6], in)), M(f->C[5], f->R[1]));
    __m128 ar = M(f->C[0], f->R[2]);
    __m128 ai = M(f->C[1], f->R[2]);
    __m128 s1 = A(M(in, f->C[2]), S(M(ar, f->R[0]), M(ai, f->R[1])));
    __m128 s2 = A(M(ai, f->R[0]), M(ar, f->R[1]));
...
inline __m128 IIR24CFCquad(QuadFilterUnitState *__restrict f, __m128 in)
{
    ...
    __m128 y2 = A(A(M(f->C[4], f->R[3]), M(f->C[6], y)), M(f->C[5], f->R[4]));
    __m128 s3 = A(M(y, f->C[2]), S(M(f->C[0], f->R[3]), M(f->C[1], f->R[4])));
    __m128 s4 = A(M(f->C[1], f->R[3]), M(f->C[0], f->R[4]));
    ...
    f->C[7] = A(f->C[7], f->dC[7]); // Clipgain
    const __m128 m01 = F(0.1f);
    f->R[2] = MAX(m01, S(m128_one, M(f->C[7], M(y2, y2))));
...
inline __m128 IIR24CFLquad(QuadFilterUnitState *__restrict f, __m128 in)
{
    ...
    __m128 y2 = A(A(M(f->C[4], f->R[3]), M(f->C[6], y)), M(f->C[5], f->R[4]));
    __m128 s3 = A(M(y, f->C[2]), S(M(ar, f->R[3]), M(ai, f->R[4])));
    __m128 s4 = A(M(ai, f->R[3]), M(ar, f->R[4]));
...
inline __m128 IIR24Bquad(QuadFilterUnitState *__restrict f, __m128 in)
{
    ...
    f->R[0] = M(f1, f->R[4]);
    f->R[1] = M(g1, f->R[4]);
    __m128 y1 = A(A(M(f->C[6], g2), M(f->C[5], g1)), M(f->C[4], f1));

    f2 = S(M(f->C[3], y1), M(f->C[1], f->R[3])); // Q2*in - K2*R1
    g2 = A(M(f->C[1], y1), M(f->C[3], f->R[3])); // K2*in + Q2*R1
    f1 = S(M(f->C[2], f2), M(f->C[0], f->R[2])); // Q1*f2 - K1*R0
    g1 = A(M(f->C[0], f2), M(f->C[2], f->R[2])); // K1*f2 + Q1*R0
    f->R[2] = M(f1, f->R[4]);
    f->R[3] = M(g1, f->R[4]);
    __m128 y2 = A(A(M(f->C[6], g2), M(f->C[5], g1)), M(f->C[4], f1));

    f->C[7] = A(f->C[7], f->dC[7]); // Clipgain
    const __m128 m01 = F(0.1f);
    f->R[4] = MAX(m01, S(m128_one, M(f->C[7], M(y2, y2))));
...
{
    f->C[0] = A(f->C[0], f->dC[0]);
    f->C[1] = A(f->C[1], f->dC[1]);
    f->C[2] = A(f->C[2], f->dC[2]);

    f->R[0] = basic_blocks::dsp::softclip8_ps(
        A(f->R[0], M(f->C[1], S(S(M(in, f->C[0]), M(f->C[2], A(f->R[3], f->R[4]))), f->R[0]))));
    f->R[1] = A(f->R[1], M(f->C[1], S(f->R[0], f->R[1])));
    f->R[2] = A(f->R[2], M(f->C[1], S(f->R[1], f->R[2])));
    f->R[4] = f->R[3];
    f->R[3] = A(f->R[3], M(f->C[1], S(f->R[2], f->R[3])));
...
__m128 COMBquad_SSE2(QuadFilterUnitState *__restrict f, __m128 in)
{
    ...
    const __m128 m256 = F(256.f);
    const __m128i m0xff = _mm_set1_epi32(0xff);

    f->C[0] = A(f->C[0], f->dC[0]);
    f->C[1] = A(f->C[1], f->dC[1]);

    __m128 a = M(f->C[0], m256);
    ...
    return A(M(f->C[3], DBRead), M(f->C[2], in));
}
