...
inline double Map4PoleResonance(double reso, double freq, int subtype)
{
    ...
    case st_Medium:
        reso *= max(0.0, 1.0 - max(0.0, (freq - 58) * 0.05));
        return 0.99 - 0.9949 * std::clamp((double)reso, 0.0, 1.0);
    case st_Driven:
        reso *= max(0.0, 1.0 - max(0.0, (freq - 58) * 0.05));
        return (1.0 - 1.05 * std::clamp((double)reso, 0.001, 1.0));
    default:
    case st_Clean:
        return (2.5 - 2.3 * std::clamp((double)reso, 0.0, 1.0));
    }
}
